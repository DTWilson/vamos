...
Here we include some code to generate figures for our BMBR application around multiple outcomes.
...
ggplot(u_ps, aes(a,b,colour = g)) + geom_point() + 
  scale_colour_discrete(name="Decision", labels=c("Stop", "Go")) +
  ylab("Utility parameter b") + xlab("Utility parameter a") +
  theme_minimal()

#ggsave("tree_points_plot.png", height = 3, width = 5 )
...
m0 <- c(0.3, 0.3)
S0 <- 2*matrix(c(0.3^2, 0.02, 0.02, 0.3^2), ncol = 2)
...
#saveRDS(ss, "ex_ss.rds")
#saveRDS(fit, "ex_fit.rds")
```


```{r}
ss <- readRDS("ex_ss.rds")
fit <- readRDS("ex_fit.rds")
...
ggplot(ss_sub, aes(x, y)) + geom_point(alpha = 0.3, colour = "red") +
  geom_point(data = ss[ss$h != "null",][1:500,], alpha = 0.3, colour = "grey30") +
  #geom_point(data = ss_sub[501,], size = 2, colour = "black") + 
  stat_contour(data = cont_grid, aes(z = p)) +
  ylab(expression("Model parameter" ~ theta[2])) + 
  xlab(expression("Model parameter" ~ theta[1])) + 
  theme_minimal()

ggplot(ss_n, aes(x = p)) + geom_histogram(fill = "grey50", colour = "black") +
  theme_minimal()

#ggsave("pow_func.png", height = 3, width = 5 )
```
...
# Sample from the prior
s <- rmvnorm(10^7, mean = m0, sigma = S0)
s <- as.data.frame(s)
...
Want a figure showing the maxes histogramed and the inferred predictive distribution of the block maximum.

```{r}
y <- pgev_cp(maxes[!is.na(maxes)])

df <- data.frame(m = maxes,
                 d = y$ml_cdf)

y <- rgev_cp(10^6, maxes[!is.na(maxes)])

# 0.2749317

ggplot(df, aes(m)) + geom_histogram(fill = "grey70", colour = "black") + 
  geom_density(data=data.frame(m = y$cp_deviates), colour = "red", linewidth = 1) +
  xlab("Block maximum type I error rate") + ylab("Density") +
  geom_segment(aes(x = 0.2749317, y = 0, xend = 0.2749317, yend = 30), linetype = 2, linewidth = 1) +
  annotate("text", x = 0.2749317, y = 33, label = "0.275") +
  theme_minimal()

#ggsave("max_hist.png", height = 3, width = 5 )
```
