...
To allow for these kind of preferences we can change the way we set up our hypotheses. We propose to construct these by first choosing lower and upper limits beyond which trade-offs are not considered, denoted $c_1$ and $c_2$. We then define a value contour which lies on the point $(0, c_2)$ and then use this to construct our null and alternative hypotheses by shrinking or expanding this contour with respect to the point $(c_1, c_2)$.
...
$$
c_1 - \frac{c_1 - z_1}/s < 0 ~\text{ and }~ c_2 - (c_2 - z_2)/s < n_2.
$$
...
hyp_plot <- function(a, c_x, c_y = c_x, n_y = 0, b_null = 0, b_alt = 2.486475) {
  ...
  # hypotheses.
  df$h <- ifelse(c_y - (c_y - df$y)/s_n < (w*c_y - (c_x - (c_x - df$x)/s_n))/(w + w*a*(c_x - (c_x - df$x)/s_n)) | c_x - (c_x - df$x)/s_n < 0 | c_y - (c_y - df$y)/s_n < n_y, 0, 
                 ifelse(c_y - (c_y - df$y)/s_a > (w*c_y - (c_x - (c_x - df$x)/s_a))/(w + w*a*(c_x - (c_x - df$x)/s_a)) & c_x - (c_x - df$x)/s_a > 0 & c_y - (c_y - df$y)/s_a > n_y, 2, 1))
