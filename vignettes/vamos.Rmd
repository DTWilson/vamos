...
Although not known, the unknown correlation, along with all the other model parameters, can be estimated. In principle we could use these estimates when constructing the critical region, but in practice this is probably too complicated to work out in closed form. 

If we move to a Bayesian framework then we aren't reliant on pre-specified rules based on sampling distributions - we can use the posterior to make decisions instead. We can take the same form of decision rule and look at our posterior over it, deciding in favour if we are more likely to be above than below. We can estimate that probability using the Monte Carlo method. As before the decision rule can be parametrised to make it more or less conservative.
